...
UPLOAD_FOLDER_VDD = join(dirname(realpath(__file__)), 'static/uploads/') # where uploaded files are stored
ALLOWED_EXTENSIONS = set(['png', 'PNG', 'jpg', 'JPG', 'jpeg', 'JPEG', 'gif', 'GIF']) # models support png and gif as well

app = Flask(__name__)
app.config['UPLOAD_FOLDER_VDD'] = UPLOAD_FOLDER_VDD
...
def upload_and_classify():
	if request.method == 'POST':
		...
		if file and allowed_file(file.filename):
			...
			file.save(os.path.join(app.config['UPLOAD_FOLDER_VDD'], filename))
...
def send_file(filename):
    return send_from_directory(UPLOAD_FOLDER_VDD, filename)
...
@app.route('/uploads/<filename>')
def uploaded_file(filename):
	return send_from_directory(app.config['UPLOAD_FOLDER_VDD_VDD'],
							   filename)
